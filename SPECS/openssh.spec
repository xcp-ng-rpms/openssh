...
# XCP-ng BEGIN Disable SELinux and define the sshd user & group ID
%global WITH_SELINUX 0
...
# XCP-ng BEGIN XCP-ng does not have gnome, so we disable it in previous version.
...
Name: openssh
Version: %{openssh_ver}
Release: 27.%{xcpng_subrel}~xcpng3707.8%{?dist}
...
# XCP-ng BEGIN XCP-ng does not have this file in the tarball.
...
# XCP-ng backports of upstream patches
...
# XCP-ng BEGIN XCP-ng doesn't have this package
...
# XCP-ng BEGIN Remove this BuildRequires as we don't use x11 forwarding
#BuildRequires: xauth
# XCP-ng END
...
# XCP-ng BEGIN XCP-ng does not have that, and we don't support it.
...
# XCP-ng BEGIN XCP-ng has its own rules; let's not package rules that could conflict with them.
...
%post clients
# XCP-ng BEGIN %%systemd_user_post is not usable on XCP-ng 8.3, so the
# output of `rpm --eval %%systemd_user_post` is duplicated here, adapted
# for OpenSSH.
...
# XCP-ng BEGIN Not marked %%config on purpose: this is the XCP-ng policy file and
...
%changelog
* Fri Sep 25 2026 Lucas Ravagnier <lucas.ravagnier@vates.tech> - 9.9p1-30.2
- XCP-ng must not package redhat confs and must apply its own.
- Create the initial XCP-ng configuration with post-quantum support.
- Harden the cryptographic algorithms to reduce the attack surface.
- Disable PrintMotd because PAM now manage it.
- Set MaxAuthTries to 3 to mitigate brute-force attacks. (ANSSI recommendation)
- Disable X11Forwarding because XCP-ng does not support X11 forwarding.
- Link against the actual sftp-server implementation.
...
- The %systemd_user_post macro was not usable, so its expansion is
  duplicated inline in %post clients
